...
class BigQueryOperator:
    ...
    def _env_check(self):
        ...
        try:
            ...
            os.environ[
                self.required_env_var
            ]  # Throws error if required environment variable is not set
            return bigquery.Client()
        except Exception as e:
            print(
                "You have not set the required GOOGLE_APPLICATION_CREDENTIALS environment variable.\n"
    ...
    @staticmethod
    def _query_job(
        job_instance, create_bq_storage_client: bool = True, silent: bool = False
    ):
        ...
        while job_instance.state == "RUNNING":
            ...
            data = job_instance.result().to_dataframe(
        ...
        if silent is False:
            ...
            print(
                "This query will bill {} bytes.".format(job_instance.total_bytes_billed)
            )
            print(
                "This query will process {} bytes.".format(
                    job_instance.total_bytes_processed
                )
            )
    ...
    def bigquery_download_local(
        ...
        if sql is None:
            raise SqlNotSet(
                "SQL",
                "A SQL query must be defined",
                self.bigquery_download_local.__name__,
            )

        query_job = self.client_gbq.query(sql)

        data = self._query_job(
            job_instance=query_job,
            create_bq_storage_client=create_bq_storage_client,
            silent=silent,
        )

        write_output_func = configure_output(
            location="local", return_type=data_return_type
        )

        write_output_func(
            data=data, output_location=output_path, params=custom_output_params
        )

        return

    def bigquery_upload_gcs(
        self,
        sql: str = None,
        data_return_type: str = "text",
        custom_output_params: Optional[dict] = None,
        ...
        if sql is None:
            raise SqlNotSet(
                "SQL", "A SQL query must be defined", self.bigquery_upload_gcs.__name__
            )
        ...
        write_output_func = configure_output(
            location="gcs", return_type=data_return_type
        )
        write_output_func(
            data=data,
            bucket_name=gcs_bucket_name,
            destination_blob_name=gcs_destination_blob_name,
            params=custom_output_params,
        )
        return

    def bigquery_extract_gcs(
        self,
        sql: str = None,
        ...
        if sql is None:
            raise SqlNotSet(
                "SQL", "A SQL query must be defined", self.bigquery_extract_gcs.__name__
            )
    ...
    def bigquery_sql_operator(self, sql: str = None, silent: Optional[bool] = False):
        ...
        if sql is None:
            raise SqlNotSet(
                "SQL",
                "A SQL query must be defined",
                self.bigquery_sql_operator.__name__,
            )
    ...
    def bigquery_upload_stream(self, table_id=None, rows=None):
        """
        New rows must be json formatted
        """

        rows_to_insert = [rows]
        errors = self.client_gbq.insert_rows_json(
